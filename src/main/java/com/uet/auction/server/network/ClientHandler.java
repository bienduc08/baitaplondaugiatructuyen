...
public class ClientHandler implements Runnable {
            ...
            while (true) {
                ...
                switch (request.getType()) {
                    ...
                    case "APPROVE_PRODUCT":
                        ...
                        // Set APPROVED (không phải OPEN) để AuctionTimer tự mở đúng start_time
                        response = auctionService.changeProductStatus(pApprove.getId(), "APPROVED");
                        sendResponse(response);
                        if (response.isSuccess()) {
                            SocketServer.broadcastToLoggedInUsers(new AuctionResponse(true, "UPDATE_PRICE", null));
                        }
    ...
    /**
     * Trả về username của user đang đăng nhập trên kết nối này.
     * Trả về null nếu client chưa đăng nhập (đang ở màn hình Login).
     * Dùng bởi SocketServer.broadcastToLoggedInUsers() và sendToUser().
     */
